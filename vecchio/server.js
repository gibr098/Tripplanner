...
app.get('/api/:luoghi/:citta', function (req, res) {
    request({
        ...
        if (error) {
            ...
            var tot = info.results.length;
            if (tot == 0) {
                var errore = "Nessun risultato ottenuto";
                res.send(errore);
            }
            else {
                var ris=[];
        ...
        if (error) {
            ...
            if (info.results.length == 0) {
                ...
                for (var i = 0; i < tot; i++) {
                    ...
                    //database.salva(citta,info.results[i].name,info.results[i].formatted_address);
                }
